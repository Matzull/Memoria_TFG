...
El sistema APRS utiliza tramas AX.25 para la transmisión de datos a través de las ondas de radio. Estas tramas contienen información sobre posición, velocidad, dirección y otros datos emitidos por estaciones de radioaficionados, objetos móviles y estaciones fijas.

\subsubsection*{Formato de la Trama AX.25}

Como se muestra en la \Cref{fig:aprs-frame} se puede ver que .

\begin{figure}[h]
	\centering
	\includegraphics[width=1\textwidth]{Imagenes/Chapter_4/AX.25Frame.png}
	\caption[Formato de trama AX.25.]{Formato de trama AX.25 (elaboración propia).}
	\label{fig:aprs-frame}
\end{figure}

\subsubsection*{Contenido de la Trama}

Dentro del campo de información de una trama AX.25, se pueden incluir diversos datos relevantes para APRS. Esto puede incluir información de posición (latitud y longitud), velocidad, altitud, rumbo, comentarios del operador, mensajes de texto, etc.

\[\underbrace{\textbf{\textcolor{red}{NOCALL}}}_{Origen} : \underbrace{\textbf{\textcolor[HTML]{FA9F42}{APRS}}}_{Destino}, \underbrace{\textbf{\textcolor{blue}{OH7RDA, OH7RDB}}}_{Digipeaters\ y\ I-Gates} : \underbrace{\textbf{\textcolor{green}{!4903N\/07201W-Test \/A=001234}}}_{Campo\ datos}\]
...
\begin{figure}[!h]
	\centering
	\includegraphics[width=0.85\textwidth]{Imagenes/Chapter_4/db_diagram.png}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\textwidth]{Imagenes/Chapter_4/first_draft.png}
...
En esta sección se describe la arquitectura del servidor web que aloja la aplicación Dash utilizando Flask como backend, junto con Gunicorn y Nginx como reverse proxy.
...
Gunicorn es un servidor HTTP WSGI (Web Server Gateway Interface) para Python. Se utiliza para servir la aplicación Flask de forma eficiente y escalable. Gunicorn gestiona múltiples procesos de forma paralela, lo que mejora el rendimiento de la aplicación. Para APRSINT se ha configurado Gunicorn para ejecutar la aplicación Flask en modo demonio y con 4 procesos de trabajo. Gunicorn crea un socket UNIX en el que escucha las solicitudes HTTP y las reenvía a la aplicación Flask para su procesamiento.

\subsubsection*{Nginx}

Nginx es un servidor web de código abierto que en este caso se ha utilizado como proxy inverso. Actúa como intermediario entre los clientes y el servidor Flask/Gunicorn. Nginx se encarga de recibir las solicitudes HTTP, dirigirlas al servidor Gunicorn y devolver las respuestas a los clientes. Nginx maneja tareas como el balanceo de carga, el almacenamiento en caché la compresión de archivos y la seguridad.
...
Este proceso asegura que las solicitudes sean manejadas de manera eficiente y escalable, garantizando una experienci de navegación más fluida.
